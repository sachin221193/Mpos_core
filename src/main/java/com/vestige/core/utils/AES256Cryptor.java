...
public class AES256Cryptor {
    ...
    public static String encrypt(String plaintext, String passphrase) {
        try {
            ...
            EvpKDF(passphrase.getBytes(UTF), keySize, ivSize, saltBytes, key, iv);

            // Actual encrypt
            Cipher cipher = Cipher.getInstance(AES);
            cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "AES"), new IvParameterSpec(iv));
            byte[] cipherBytes = cipher.doFinal(plaintext.getBytes(UTF));
             ...
             */
            byte[] sBytes = "Salted__".getBytes(UTF);
    ...
    public static String decrypt(String ciphertext, String passphrase) {
        try {
            ...
            byte[] ctBytes = Base64.getDecoder().decode(ciphertext.getBytes(UTF));
